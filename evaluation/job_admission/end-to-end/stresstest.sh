...
Options:
 ...
 --ramp-step-size <n>            Number of steps per ramp-up/-down step (default: $RAMP_STEP_SIZE)
...
Note:
  ...
  For 'ramp-test', --num-jobs controls the number of jobs per --iterations.
EOF
}
...
spec:
  ...
  template:
    spec:
      containers:
      - name: stresstest
        image: harbor.pt.horizon-opencube.eu/vni-system/alpine:latest
        command: ["echo"]
      restartPolicy: Never
      nodeSelector:
...
function spikeTest() {
  ...
  # deliberately silence stderr - we use TTLs on the jobs, so the jobs in LAUNCHED_JOBS will
  #  at some point no longer exist & kubectl will get mad at this. We still need it tough as we
  #  don't know just how long all LAUNCHED_JOBS take.
  $KUBECTL -n "$NAMESPACE" wait --timeout=-1s --for=condition=Complete "${LAUNCHED_JOBS[@]}" 1>/dev/null 2>&1
  echo "Done. Fetching job data and writing to disk."
  $KUBECTL -n "$NAMESPACE" get events -o json|gzip > "$BASEPATH/$iteration-job_output.json.gz"
...
$KUBECTL create namespace "$NAMESPACE" 1>/dev/null 2>&1
echo "Deleting events"
$KUBECTL -n "$NAMESPACE" delete events --all --timeout=10s 1>/dev/null &
sleep 10
