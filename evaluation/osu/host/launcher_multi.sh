...
LONGOPTS=help,basedir:,mpirun:,mpi-args:,binary:,binary-args:,iterations:,hostfile:,verbose,
OPTIONS=
...
Options:
 --basedir      <path>    Path to base directory for storing benchmark results.
 --hostfile     <path>    Path to hostfile for mpirun
 --mpirun       <path>    Path to mpirun
 --mpi-args     <args>    Arguments passed to mpirun
 --binary       <path>    Path to benchmark binary
 --binary-args  <args>    Arguments passed to benchmark binary
 --iterations   <N>       Number of iterations to run benchmark
 --help
 --verbose

The launcher will populate data in a directory at '<basedir>/measurements_<hostname>_<binary_name>_<date>'.
In that directory, a 'meta.md' file will be populated with the benchmark parameters.
The measurement files will be stored with format 'measurement_<i>.dat' for all 'i' iterations.
